...
export default function DoctorDashboard() {
  ...
  const [recommendations, setRecommendations] = useState(
    patient?.details.doctorsRecommendations || []
  );
  ...
  const handleLogout = () => {
  // Remove token (if you are using authentication)
  localStorage.removeItem("token");

  // Redirect to login page
  window.location.href = "/login"; // or use react-router's navigate if available
};

   const DoctorProfileDropdown = () => (
    <div className="absolute top-full right-0 mt-2 w-64 bg-white border rounded-xl shadow-lg z-50 p-4">
      <h3 className="font-bold text-gray-800">Dr. Emily Carter</h3>
      <p className="text-sm text-gray-500 mb-2">Cardiologist</p>
      <p className="text-sm text-gray-600 mb-2">Email: emily.carter@example.com</p>
      <p className="text-sm text-gray-600 mb-2">Phone: +1 234 567 890</p>
      <button
        onClick={handleLogout}
        className="w-full mt-2 px-3 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700 transition-colors"
      >
        Logout
      </button>
    </div>
    ...
    <div className="animate-fade-in">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-6">
          {/* AI Prediction */}
          <div
            className={`p-6 rounded-2xl shadow-sm border-2 ${riskClasses.bg} ${riskClasses.border}`}
          >
            <div className="flex items-center gap-3 mb-4">
            ...
            {Object.values(mockPatientData).map((p) => {
              ...
              return (
                ...
                >
                  ...
                  <div
                    className={`px-4 py-3 rounded-xl border-2 text-center ${pRiskClasses.bg} ${pRiskClasses.border}`}
                  >
                    <div className="flex items-center justify-center gap-2">
                      {p.aiPrediction.risk === "High" ? (
                        <AlertCircle
                          className={`w-5 h-5 ${pRiskClasses.text}`}
                        />
                      ) : (
                        <CheckCircle2
                          className={`w-5 h-5 ${pRiskClasses.text}`}
                      ...
                      <span
                        className={`text-sm font-semibold ${pRiskClasses.text}`}
                      >
                        AI Risk Assessment
                      </span>
                    </div>
                    <div
                      className={`text-2xl font-bold mt-1 ${pRiskClasses.text}`}
                    >
                      {p.aiPrediction.risk}
                    </div>
                  </div>
                ...
                <ul className="space-y-4">
                  {appointmentsOnSelectedDate.map((pid) => {
                    ...
                    return (
                      <li
                        key={pid}
                        className="bg-white p-4 rounded-2xl shadow-sm border border-gray-200 flex justify-between items-center cursor-pointer hover:bg-gray-50 transition-colors"
                      ...
                      >
                        ...
                        <div
                          className={`px-4 py-1.5 rounded-full text-sm font-semibold ${pRiskClasses.bg} ${pRiskClasses.text}`}
                        >
          ...
          <div className="flex items-center gap-4 relative" ref={profileRef}>
            ...
            >
              <img
                src={doctorImage}
                alt="Doctor"
                className="w-11 h-11 rounded-full ring-2 ring-offset-2 ring-blue-500 shadow-sm"
              />
